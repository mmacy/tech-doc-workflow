...
const DocumentTypeProfilesTab: React.FC<DocumentTypeProfilesTabProps> = ({ profiles, onProfilesChange, disabled }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-semibold text-theme-primary mb-1">Document type profiles</h3>
      ...
      <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
        <button
          ...
          disabled={disabled}
          className="px-4 py-2 btn-theme-primary rounded-md font-medium text-sm disabled:opacity-50 flex items-center justify-center"
        >
      ...
      {profiles.length === 0 && (
        <p className="text-theme-muted italic py-4">No document type profiles configured. Click "Add profile" to create one.</p>
      )}

      <div className="space-y-4">
        {profiles.map(profile => (
          <div key={profile.id} className="bg-theme-elevated p-4 rounded-lg border border-theme">
            <div className="flex justify-between items-start">
              <div>
                <h4 className="text-lg font-semibold text-theme-accent">{profile.name}</h4>
                <p className="text-sm text-theme-secondary mt-1 mb-2 whitespace-pre-wrap">{profile.description}</p>
                <details className="text-xs text-theme-muted mb-2">
                    <summary className="cursor-pointer hover:text-theme-accent">View document type guidance</summary>
                    <pre className="mt-1 p-2 bg-theme-surface rounded text-xs whitespace-pre-wrap max-h-40 overflow-y-auto scrollbar-theme">{profile.docTypeDescription || "No guidance defined."}</pre>
                </details>
                <details className="text-xs text-theme-muted">
                    <summary className="cursor-pointer hover:text-theme-accent">View template</summary>
                    <pre className="mt-1 p-2 bg-theme-surface rounded text-xs whitespace-pre-wrap max-h-40 overflow-y-auto scrollbar-theme">{profile.template || "No template defined."}</pre>
                </details>
              </div>
              <div className="flex space-x-2 shrink-0 ml-4">
                <button onClick={() => handleEditProfile(profile)} disabled={disabled} className="text-theme-accent hover:text-theme-accent/80 disabled:text-theme-muted p-1" title="Edit profile">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                </button>
                <button onClick={() => handleDeleteProfile(profile.id)} disabled={disabled} className="text-theme-error hover:text-theme-error/80 disabled:text-theme-muted p-1" title="Delete profile">
